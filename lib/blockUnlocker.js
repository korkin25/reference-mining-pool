				...
				*/
                ...
                if (block.workerShares) {
                    ...
                    Object.keys(block.workerShares).forEach(function (worker) {
                        ...
                        unlockedBlocksCommands.push(['zadd', config.coin + ':blockstat:' + worker, block.height, [block.time,now,block.reward,block.workerShares[worker],totalShares,workerReward,Math.round(feeRecovery/blocks.length),0,block.devfee,0,block.difficulty].join(':')]);
                    });
                }
            });
